...
{if isset($order_collection)}
    ...
    {if isset($transformSuccess)}
		<p class="alert alert-success">{l s='Your guest account has been successfully transformed into a customer account. You can now login as a registered shopper. '} 
        	<a href="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" title="{l s='page.'}">{l s='page.'}</a>
        </p>
	{else}
		<form method="post" action="{$action|escape:'html':'UTF-8'}#guestToCustomer" class="std">
			<fieldset class="description_box box">
                ...
                <p>
                	<strong class="dark">{l s='Transform your guest account into a customer account and enjoy:'}</strong>
                </p>
				<ul>
                ...
                <div class="row">
                	<div class="col-xs-12 col-sm-5 col-md-4 col-lg-3">
                        <div class="text form-group">
                            <label>
                            	<strong class="dark">{l s='Set your password:'}</strong>
                            </label>
				...
				<p>
                    <button type="submit" name="submitTransformGuestToCustomer" class="button button-medium btn btn-default">
                    	<span>
                        	{l s='Send'}
...
{else}
	...
	{if isset($show_login_link) && $show_login_link}
		<p>
        	<img src="{$img_dir}icon/userinfo.gif" alt="{l s='Information'}" class="icon" />
            <a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" title="{l s='Click here to login to your customer account.'}">
	...
	<form method="post" action="{$action|escape:'html':'UTF-8'}" class="std" id="guestTracking">
		<fieldset class="description_box box">
			...
			<p>
                <button type="submit" name="submitGuestTracking" class="button btn btn-default button-medium">
                	<span>
                    	{l s='Send'}
                        <i class="icon-chevron-right right"></i>
                    </span>
